...
import { runImportArticleBodyFromSubmission } from "@/lib/manuscript/import-article-body-from-submission";
...
export async function POST(req: Request) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const result = await runImportArticleBodyFromSubmission(supabase, articleId, (p) => {
