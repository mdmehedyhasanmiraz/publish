...
export default async function AdminSettingsPage() {
  ...
  const [{ data: publishers }, { data: emailEvents }, { data: openalexMatches }, { data: auditLogs }] = await Promise.all([
    supabase.from("publishers").select("id, name, slug").limit(10),
  ...
  return (
    <div>
      ...
      <div className="mt-5 rounded border bg-white p-4">
        <h3 className="text-sm font-semibold">Publisher Configuration</h3>
        <div className="mt-3 grid gap-2">
          {(publishers ?? []).length ? (
            (publishers ?? []).map((p) => (
              <div key={p.id} className="rounded border p-3 text-sm">
                <p className="font-medium">{p.name}</p>
                <p className="text-muted-foreground">slug: {p.slug}</p>
              </div>
            ))
          ) : (
            <p className="text-sm text-muted-foreground">No publisher rows found.</p>
          )}
        </div>
      </div>
