-- Handling editor picker: show name as profiles.first_name + profiles.last_name (not display_name / middle).
      ...
      coalesce(
        nullif(
          trim(
            concat_ws(
              ' ',
              nullif(trim(coalesce(p.first_name::text, '')), ''),
              nullif(trim(coalesce(p.last_name::text, '')), '')
            )
          ),
          ''
        ),
